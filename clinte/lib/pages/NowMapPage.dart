...
class _NowMapPageState extends State<NowMapPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _downloadedImagePath != null
          ? Center(
              child: Stack(
                children: [
                  Image.file(
                              ...
                              _markers.add(
                                Marker(
                                  ...
                                  infoWindow: InfoWindow(
                                    ...
                                    snippet: data['result']['formatted_address'],
                                  ),
                                ),
                              );
                            });
                      ...
                      onPressed: () {
                        if (_searchController.text.isNotEmpty) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Please select a place to download.')),
                          );
                        }
                      },
  ...
  Future<void> _downloadMap(String placeName) async {
    final position = await _mapController?.getLatLng(const ScreenCoordinate(x: 300, y: 200));
    final zoom = await _mapController?.getZoomLevel();

    if (position == null || zoom == null) return;

    FileDownloader.downloadFile(
      url: 'https://maps.googleapis.com/maps/api/staticmap?center=${position.latitude},${position.longitude}&zoom=$zoom&size=600x400&markers=color:red|${position.latitude},${position.longitude}&key=$_googleApiKey',
      ...
      onDownloadCompleted: (path) async {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Map downloaded!')),
        );
      },
